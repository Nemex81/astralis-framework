...
### FASE 3: SCANSIONE A CASCATA DEI PROGETTI (WORKSPACE TRIAGE)
...
### FASE 4: FUSIONE INTEGRATIVA NEI PROGETTI FIGLI (LIVELLO 2)
Per ciascun progetto da aggiornare:
1. **Preservazione Integrale `knowledge/` (Binario A)**:
   - Mantieni al 100% tutte le schede storiche create nelle sessioni di Auto-Apprendimento;
2. **Allineamento `GEMINI.md` Locale**:
   - Aggiorna il `GEMINI.md` del progetto inserendo l'Indice Ragionato con micro-sommari per ogni scheda locale e l'Isola Protetta per le regole del repository;
3. **Changelog Retroattivo**:
   - Se manca `CHANGELOG.md`, crealo ricostruendo la cronologia da `git log` o inizializzalo da template;
4. **Verifica `README.md`**:
   - Assicurati che il `README.md` pubblico di root rimanga dedicato esclusivamente alla documentazione del software.

---

### FASE 5: AUDIT & VALIDAZIONE DI CONFORMITÀ POST-AGGIORNAMENTO
1. **Self-Check Automatico**:
   - Assenza di contraddizioni tra regole nuove e custom;
   - Assenza di regole duplicate/riverberate tra Livello 0, 1 e 2;
   - Integrità di tutti i link e riferimenti;
2. **Ricevuta Finale per NVDA**:
   - Emetti la **Ricevuta di Conformità Post-Aggiornamento** con l'elenco dei file e dei progetti aggiornati con successo.
